...
agent = Agent(action_size=3,lr=0.00025,mem_size=9000,conv=False)
...
while (i<n_games):
    ...
    while not done:
        ...
        action_idx = agent.choose_action(observation[1])
        ...
        state = np.expand_dims(observation[1],axis=0)
        new_state = np.expand_dims(observation_[1],axis=0)
